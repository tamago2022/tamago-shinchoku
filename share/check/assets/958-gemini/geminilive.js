/* ─────────────────────────────────────────────────────────────
 * これは Google の公式サンプルを、そのまま持ってきたものです。
 *
 *   出どころ: https://github.com/google-gemini/gemini-live-api-examples
 *             gemini-live-ephemeral-tokens-websocket/frontend/geminilive.js
 *   ライセンス: Apache-2.0（Google LLC）
 *
 * ★ここは触らない。直したくなったら、上の公式を見て直す。
 *   このページ側で変えているのは、
 *     ・つなぎ先（serviceUrl）を公式ドキュメントどおり v1beta にも切り替えられるようにした
 *   の1点だけで、それは 958-gemini-live-tamago.html の側でやっています。
 * ───────────────────────────────────────────────────────────── */
...
class GeminiLiveAPI {
  ...
  async onReceiveMessage(messageEvent) {
    ...
    try {
      ...
      // ★ここだけ足した：値段を実測で出すため、生のメッセージも上へ渡す
      if (this.onRawMessage) this.onRawMessage(messageData);
    } catch (err) {
