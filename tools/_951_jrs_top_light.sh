#!/usr/bin/env bash
# 【951番・2026-09-19】トップが11〜50秒かかる件（曲データ1本6.5MBを全員に毎回送っている）を直す。
#
# なぜ要るか：Cowork のサンドボックスには joy-relief-station がマウントされておらず、
#   GitHub の資格情報も無い。_930 / _936 / _937 / _938 と同じ理由・同じ形。
#   ホスト側で動く command_ingest.py（joy_push）から1回だけ呼んでもらう。
#
# ★他の番号のセッションと joy_push の口を共有しているので **奪わない**。
#   _930 側は「951が idle でなければ 951 を1回走らせてから、いつも通り先へ進む」。
#   仕事が終わったら status/_951/PHASE に idle と書けば口は完全に元通りになる。
#
# PHASE（status/_951/PHASE）:
#   idle … 何もしない（既定）
#   run  … status/_951/run_step.sh をホスト側で実行する（調査・適用・計測・公開すべてここで切り替える）
set -uo pipefail
export PATH="/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:$PATH"

REPO="/Users/mac/Desktop/tamago-shinchoku"
WORK="${REPO}/status/_951"
LOG="${WORK}/run.log"
mkdir -p "${WORK}"
