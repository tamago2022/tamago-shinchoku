...
ROUTE = {
    ...
    "chappy": {
        ...
        "foot": "",
        # ★2026-09-23 実測：この口は今**残高切れで閉じている。**
        #   OpenAI API → HTTP 429 / "type":"insufficient_quota" /
        #   "code":"credit_balance_exhausted"（gpt-4o・gpt-4o-mini とも同じ）。
        #   前日（09-22）は2.4〜2.7秒で返っていたので、線でもモデル指定でもなく**お金**。
        #   課金はたまごさんの判断（不可逆の1つ）なのでこちらでは開けない。
        #   → blocked にはしない。残高が入れば**この1行のまま**また通るため。
        #     代わりの口は jules（`jules` ラベル・無料・双方向◯）。
        "note": ("APIに直接聞くので、投げたその場で返る。GitHubのbotを待たない。"
                 "★2026-09-23時点は残高切れ（429 credit_balance_exhausted）で返らない。"
                 "残高が入るまでの代わりは jules。"),
    },
    "grok": {
