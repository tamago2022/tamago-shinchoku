#!/usr/bin/env node
// 791番・4回目（経路は3回目までと同じ新規launch方式・「続けるボタン」のクリックだけforce:trueに変更）。
// 3回目のCDP相乗り(9223)は共有ブラウザが極端に混雑しタイムアウトしたため不採用。
// 2回目(fill_form2)は新規launchは成功したがボタンクリックのstability待ちでタイムアウトしたため、
// クリックだけforce:trueにして再試行する。
...
async function fill(browser, name, url, body) {
  ...
  try {
    ...
    if (c > 0) {
      await btn.first().click({ timeout: 10000, force: true }).catch((e) => log(name, "click ERR(force)", e.message));
      log(name, "STEP5 clicked continue(force)");
      await page.waitForTimeout(2500);
      log(name, "STEP5b url after click", page.url());
    }
    log(name, "STEP6 fill email");
    await page.locator("#FnReplyMail").fill(EMAIL, { timeout: 10000, force: true }).catch((e) => log(name, "email fill ERR", e.message));
    log(name, "STEP7 fill body");
    await page.locator("textarea[name='mdInput01SummaryTextarea']").fill(body, { timeout: 10000, force: true }).catch((e) => log(name, "body fill ERR", e.message));
    ...
    // 実際に入力できたか値を読み戻して確認
    const emailVal = await page.locator("#FnReplyMail").inputValue().catch(() => "(取得失敗)");
    const bodyVal = await page.locator("textarea[name='mdInput01SummaryTextarea']").inputValue().catch(() => "(取得失敗)");
    log(name, "VERIFY email=", emailVal.slice(0, 60));
    log(name, "VERIFY bodyLen=", String(bodyVal.length));
  } catch (e) {
