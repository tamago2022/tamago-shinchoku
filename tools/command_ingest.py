...
BIG_JOB_KEYWORDS = ("全件", "すべての", "全部の", "全アーティスト", "全曲", "全記事")
# 2026-09-06(585番) 「全ページ」は素の文字列一致だと外す。「全ページ戻す」「全ページ揃え」の
# ように**単発の共有コンポーネント修正の効果範囲**を言うだけの使い方もあり（452番の誤判定の
# 実例）、「全ページの◯◯を」のように**個別に数えられる対象を指す時**とは区別できないため。
# 後ろに助詞「の」が続く時（＝ページ1枚ごとに別々の対象がある言い方）だけ大きい仕事とみなす。
BIG_JOB_ALL_PAGES_RE = re.compile(r"全ページの")
# 二桁以上の「N件」でも、「10件＋高さ揃え」のようなUIパラメータの説明では対象の個数ではない
# （452番の誤判定の実例）。「N件を」「N件の」の形（＝対象を指して判定・修正する言い方）の
# 時だけ大きい仕事の合図として拾う。
...
def _is_big_job(text):
    ...
    2026-09-06 修正①：**指示文の全文を見ていたのが間違いだった。**
    たまごさんの発言をそのまま引用すると「全ページ」「82件」といった言葉が本文に入る。
    そのせいで、画面の工事1本のような小さい仕事まで「大きい仕事」と誤判定され、
    同じ題名が10本以上並ぶ事故になった（たまごさん「これぶっ壊れてるでしょ？」2026-09-06）。
    **見るのは1行目（題名）だけ。**本文の引用では割らない。

    2026-09-06 修正②（585番・再検証で見つかった穴）：①だけでは防げない実例があった。
    441番・452番の題名そのものに「全ページ」「10件」が含まれていたため、①の修正後も
    `tools/test_is_big_job.py` の再現テストで誤判定が再現した。**題名に大きい仕事っぽい
    言葉が入っていても、それが「対象は1つだけ・効果範囲の説明」なのか「対象が複数ある
    ...
    for kw in BIG_JOB_KEYWORDS:
        if kw in t:
            return True
    if BIG_JOB_ALL_PAGES_RE.search(t):
        return True
