...
def shoujiki(d, w1, w3, te, use, op=None):
    ...
    if not use.get("tried"):
        out.append("★<b>「URLを貼って落ちる確率」は、貼る所にたどり着けなかったので回数が出ていません。</b>"
                   "理由：%s。<b>『落ちなかった』ではありません。『測れていない』です。</b>"
                   % esc(use.get("skipWhy") or "不明"))
    elif use.get("basho") != "棚編集の画面":
        out.append("★<b>URL貼りを測ったのは棚編集の中ではなく「%s」です。</b>"
                   "棚編集の中の貼り口には今日たどり着けませんでした。場所が違う以上、"
                   "たまごさんが言っている「落ちる」と同じものを測れているとは限りません。" % esc(use.get("basho")))
