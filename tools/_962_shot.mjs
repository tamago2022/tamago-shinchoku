#!/usr/bin/env node
/**
 * 962番：どのURLでも撮れる、汎用の証拠スクショ（全ページ）。
 * _825_screenshot_375.mjs（ページ固有の #dekitaSec 待ちが入っている）から
 * その待ちだけを外し、幅・高さを引数で受けられるようにしたもの。
 ...
 *
 * 使い方: node tools/_962_shot.mjs <URL> <出力PNGパス> [幅] [高さ]
 */
...
if (!URL_ARG || !OUT_FILE) {
  console.error("使い方: node tools/_962_shot.mjs <URL> <出力PNGパス> [幅] [高さ]");
  process.exit(1);
}
...
const H = parseInt(process.argv[5] || "1400", 10);
...
async function main() {
  ...
  const profile = mkdtempSync(resolve(tmpdir(), "962-shot-"));
  const chrome = spawn(
    CHROME,
    [
      "--headless=new",
  ...
  try {
    ...
    // ページ固有の待ちは持たない。画像の読み込みが終わるまでだけ待つ。
    for (let k = 0; k < 40; k++) {
      const r = await send("Runtime.evaluate", {
        expression: `(() => { const a=[...document.images]; return JSON.stringify({n:a.length, ok:a.filter(x=>x.complete).length}); })()`,
        returnByValue: true,
      });
    ...
    const full = JSON.parse(size.result?.value || "{}").h || H;
    const clip = { x: 0, y: 0, width: W, height: full, scale: 1 };
