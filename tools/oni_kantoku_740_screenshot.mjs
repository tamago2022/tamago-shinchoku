#!/usr/bin/env node
/**
 * 740番：鳥の棚（かわいいの扉）にX投稿の動物動画カードを1件追加した証拠スクショ。
 *
 * 既存の安全な方式（tools/oni_kantoku_734_screenshot.mjs）をそのまま流用：
 ...
 *
 * 使い方: node tools/oni_kantoku_740_screenshot.mjs
 * 出力: share/check/img/740-*.png
 */
...
const TARGETS = [
  {
    url: "https://joy-relief-station.lovable.app/world/cute#shelf-tori",
    out: "740-1-world-cute-tori-shelf.png",
    width: 390,
    label: "かわいいの扉・鳥の棚（一覧表示）",
    scrollToId: "shelf-tori",
  },
  {
    url: "https://joy-relief-station.lovable.app/room/card/tori-vapaana-lintu",
    out: "740-2-card-tori-vapaana-lintu.png",
    width: 390,
    label: "鳥カード個別ページ",
  },
];
...
async function shootOne(target) {
  ...
  const profile = mkdtempSync(join(tmpdir(), "oni-740-shot-"));
  const chrome = spawn(
    CHROME,
    [
      "--headless=new",
