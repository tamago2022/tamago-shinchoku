#!/usr/bin/env node
/**
 * 917番【仕組み⑭】外部AI検品(gaibu_kenpin)専用の汎用スクリーンショット。
 * 既存 tools/_759_screenshot.mjs と同じ安全な方式（headless Chrome・一時プロファイル・
 * 使用後SIGKILL・たまごさんの通常ブラウザには一切触れない）をそのまま流用し、
 * 特定要素(dekitaSec等)を前提にしない汎用版として作る。
 *
 * 使い方: node tools/gaibu_kenpin_shot.mjs <URL> <出力PNGパス>
 * 終了コード: 0=成功 / 1=失敗
 */
...
const OUT_FILE = process.argv[3] ? resolve(process.argv[3]) : null;
if (!URL_ARG || !OUT_FILE) {
  console.error("使い方: node tools/gaibu_kenpin_shot.mjs <URL> <出力PNGパス>");
  process.exit(1);
}
...
async function main() {
  ...
  const profile = mkdtempSync(resolve(tmpdir(), "gaibu-shot-"));
  const chrome = spawn(
    CHROME,
    [
      "--headless=new",
  ...
  try {
    ...
    const shot = await send("Page.captureScreenshot", {
      format: "png",
      clip: { x: 0, y: 0, width: 375, height: 1400, scale: 1 },
    });
